...
export function useUpdateProfileForm() {
  ...
  async function submit(): Promise<void> {
    ...
    try {
      ...
      // store. `requiresSubscription`/`requiresCompany`/`favorites`/
      // `planLimits`/`impersonatedBy` são preservados (não vêm nesta
      // resposta, só em `/auth/me`/login) — sem isso, salvar o perfil
      // apagaria esse dado já carregado da store.
      authStore.setUser(
        toAuthUser(
          user,
          authStore.user?.favorites ?? [],
          authStore.user?.planLimits ?? null,
          authStore.user?.impersonatedBy ?? null,
        ),
        {
          requiresCompany: authStore.requiresCompany,
