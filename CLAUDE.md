...
- **`docs/design/design-system.md` é obrigatório, sem exceção.** Nenhuma cor, espaçamento, raio, tamanho ou peso de fonte é hardcoded num componente — sempre via variável SCSS de `core/styles/_variables.scss` (`$color-primary`, `$spacing-16`, `$radius-8`...), que por sua vez são aliases das custom properties definidas em `core/styles/_tokens.scss`. Precisa de um valor que não existe na escala documentada? Ele está no export de origem (`docs/design/tokens/`) — traga o valor que falta, nunca invente um novo. **`design-system.md` foi particionado em 2026-09-08** (tinha passado de 6000 linhas, carregado inteiro em toda sessão via `@`) — hoje só traz tokens/princípios fundamentais + um índice "Components"; a especificação real de cada componente/tela (variantes, achados, histórico) mora em `docs/design/components/*.md` (átomos/blocks/shell) e `docs/design/screens/*.md` (telas por módulo), listados no índice. Antes de tocar num componente/tela existente, leia o arquivo correspondente — não confie só no índice. Componente novo em `shared/components/ui/` ou `blocks/` ganha uma entrada no índice de `design-system.md` E a documentação real no arquivo particionado certo, no mesmo PR.
- **Nenhum texto de UI solto direto num `.vue`, sem exceção — regra não-negociável, 2026-08-28.** Heading, label, título de coluna, texto de botão/ação, placeholder: sempre uma chave em `core/i18n/messages/pt-BR.ts`, resolvida via `$t()` (template) ou `useI18n().t()` (`<script setup>`, pra montar array/objeto fora do template). Nunca a string em português direto no template/script. Texto que não é específico de um módulo/página (ação genérica de linha — editar/excluir/visualizar/baixar/ações —, nome de marketplace) mora no namespace `common`, não duplicado dentro de cada namespace de página — checar se já existe em `common` antes de criar uma chave nova. Dado de domínio (nome de produto, preço formatado, data, "+12%") não é texto de UI — não precisa de chave, é conteúdo, não copy. Já existe `useApiMessage` (seção 6.3 de `docs/infra/convencoes-frontend-infra.md`) pra mensagem vinda do backend (`ApiMessageKey`/`NotificationMessageKey`) — papel diferente, mesmo mecanismo (`vue-i18n`) por trás dos dois.
