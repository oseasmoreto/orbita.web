...
- **`docs/design/design-system.md` é a fonte de verdade visual, obrigatória sem exceção** (regra não-negociável, repetida no `CLAUDE.md` raiz do repo): cor, tipografia, espaçamento, raio e as receitas de componente já documentadas ali. Todo átomo/bloco novo consome só as variáveis SCSS de `core/styles/_variables.scss` (que por sua vez só são aliases de `core/styles/_tokens.scss`) — nunca um hex/px direto no componente, nunca um valor fora da escala documentada. **`design-system.md` foi particionado em 2026-09-08** (passou de 6000 linhas, carregado inteiro em toda sessão via import automático) — hoje só traz tokens/princípios fundamentais + um índice "Components"; a receita real de cada componente/tela mora em `docs/design/components/*.md` (átomos/blocks/shell) e `docs/design/screens/*.md` (telas por módulo), listados no índice. Componente novo ganha entrada no índice de `design-system.md` E a receita real no arquivo particionado certo, no mesmo PR.
...
- **Ícones**: `@lucide/vue` pra genérico, ou o conjunto próprio do design system (`shared/components/icons/`, gerado via `npm run generate:icons` a partir de `docs/icons-regular/`/`docs/icons-duotone/`/`docs/icons-snow-ui/`) — nunca SVG solto colado no componente. `Icon.vue` é o wrapper fino em cima de qualquer uma das fontes. **Regra não-negociável, sem exceção**: importar o ícone do conjunto próprio sempre direto do módulo gerado (`import { Check } from '@/shared/components/icons/regular.generated'`), **nunca** por namespace (`shared/components/icons/index.ts` não reexporta os ícones de propósito) — um acesso `IconsRegular.Check` via namespace impede o bundler de eliminar os outros ícones do módulo e infla o chunk de ~1kB pra ~2,4MB (achado real, medido em build). Ícones de `docs/icons-snow-ui/` incluem swatches de estado de controle (`Checkbox-N`/`Toggle-N`/`Radio-N`) que não são conteúdo genérico — servem de referência pra construir `Checkbox.vue`/`Toggle.vue`/`Radio.vue` quando existirem, não pra uso solto. Detalhe completo em `docs/design/design-system.md`, seção Components → Icon.
...
- **Erro de CAMPO (`ApiError.fieldErrors`) nunca é mostrado cru — sempre via `useApiMessage().resolveFieldError(field, rule)`** (`docs/infra/convencoes-backend-infra.md` bootstrap/app.php: o backend manda `errors` chaveado pela RULE NAME que falhou, ex. `required`/`min`/`closure_validation_rule`, nunca uma frase pronta — mesmo espírito de catálogo do `ApiMessageKey`). Achado real, 2026-08-31: sem essa resolução, um EAN inválido mostrava literalmente "closure_validation_rule" embaixo do campo. Dicionário em `core/i18n/messages/pt-BR.ts` (`errors.validation.*`, com `errors.validation.byField.<campo>.<rule>` pros casos ambíguos onde uma regra `Closure` custom colapsa pro mesmo nome genérico) — todo `useXForm.ts` que popula `errors.value` a partir de `apiError.fieldErrors` passa cada mensagem por `resolveFieldError()` antes de guardar, nunca `messages[0]` direto.
...
- **Regra não-negociável, 2026-08-28 — o mesmo catálogo cobre TODO texto estático de UI, não só mensagem vinda do backend.** Escopo ampliado depois de o usuário encontrar vários textos soltos direto em componente em vez de passar pelo `vue-i18n` que já existe pra isso. Nenhum heading, label, título de coluna, texto de botão/ação ou placeholder fica como string literal num `.vue` — sempre uma chave nova em `core/i18n/messages/pt-BR.ts`, resolvida via `$t()` no template ou `useI18n().t()` no `<script setup>` (necessário quando o texto monta um array/objeto fora do template, ex.: `DataTableColumn[]`, `DropdownMenuOption[]`). Estrutura do catálogo, por namespace:
  - `common`: texto que não é específico de um módulo/página — ação genérica de linha (`editar`/`excluir`/`visualizar`/`baixar`/`ações`), nome de marketplace, e qualquer outro texto repetido entre 2+ telas. Checar se a chave já existe em `common` antes de criar uma nova em outro namespace — mesma régua de DRY já aplicada a componente/composable (seção 2).
  - Um namespace por página/módulo (ex.: `dashboard`), só com o que é específico dali.
  - `errors`/futuro namespace de notificação: papel diferente (seção 6.3 acima), resolvido por `useApiMessage`, não por `$t()` direto.
  - **O que NÃO é texto de UI, não precisa de chave**: dado de domínio — nome de produto vindo da API, preço já formatado, data, percentual tipo `"+12%"`. Isso é conteúdo, não copy; forçar isso pra dentro do catálogo confundiria "texto que o produto decide" com "dado que o produto exibe".
...
- Estrutura em `core/styles/`: `_tokens.scss` (as custom properties CSS de verdade — `:root`/`[data-theme='dark']` — importado **uma única vez**, só por `main.scss`, nunca por um componente: como são regras CSS reais, `@use` num componente duplicaria o bloco inteiro no CSS compilado daquele componente, já que cada `.vue` é uma unidade de compilação Sass separada), `_variables.scss` (só `$nome: var(--x)`, aliases finos sem nenhuma regra CSS própria), `_mixins.scss` (só `@mixin`/`@function`, idem — nenhum dos dois emite CSS sozinho), `_reset.scss`, `main.scss` (único ponto de entrada global, importa `tokens` + `reset` + `variables`).
- **`variables`/`mixins` são injetados automaticamente em todo `<style lang="scss">`, sem precisar de `@use` manual** (`vite.config.ts`/`vitest.config.ts`, `css.preprocessorOptions.scss.additionalData` — pedido direto do usuário, 2026-09-03, depois de notar a repetição em ~100 componentes). Nenhum componente novo escreve `@use '@/core/styles/variables' as *;`/`@use '@/core/styles/mixins' as *;` no topo do próprio `<style>` — `$color-primary`/`$spacing-16`/`@include focus-ring` etc. já resolvem sozinhos. **Só esses dois entram no `additionalData`, nunca `tokens`/`reset`** — os dois têm CSS de verdade (`:root {...}`/reset de elemento), injetar globalmente duplicaria esse CSS em CADA componente compilado; `tokens`/`reset` continuam exclusivos de `main.scss`. `vitest.config.ts` precisa da MESMA config (arquivo separado de propósito, não herda `vite.config.ts`) — sem isso, montar um componente em teste falha a compilar por variável/mixin indefinida.
- Componentes usam **scoped styles** (`<style scoped lang="scss">`) por padrão — SCSS global só pra tokens de design (variáveis/mixins, injetados como acima) e reset.
