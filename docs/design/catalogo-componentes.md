...
| Date Picker | `DatePicker.vue` ✅ | **Concluído em 2026-08-28 (Tier 11), corrigido no mesmo dia contra captura real do Figma.** Construído sobre `Popover`+`Calendar` standalone da Reka UI, não a família composta `DatePicker*` (que embute campo segmentado dia/mês/ano, sem caso de uso hoje). Model público é ISO `YYYY-MM-DD` (nunca o `DateValue` de `@internationalized/date`, que virou dependência direta do `package.json`). Fecha ao selecionar um dia (mesmo padrão do `Select.vue`). **Correção do usuário**: a primeira versão (sem grounding, API do Figma sob rate limit) tinha um layout errado — trocava o ícone à direita por um botão de limpar. A referência real mostra `CalendarBlank` fixo à esquerda + texto + `CaretUpDown` fixo à direita (mesmo padrão do `Select`), sem botão de limpar dedicado — limpar é reclicar o dia já selecionado (deseleção nativa da Reka UI). Detalhe completo em design-system.md, seção Components → DatePicker. |
